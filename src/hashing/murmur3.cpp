...
murmurhash3_x86_32(const byte *data, uint32 len, uint32 seed) {
    ...
    const uint8* tail = reinterpret_cast<const uint8*>(data + l * 4); // last 8 byte chunk of `key'; // tail - last 8 bytes
    ...
    // remainder
    switch (len & 3) { // `len % 4'
    case 3: k ^= static_cast<uint32>(tail[2] << 16);
    case 2: k ^= static_cast<uint32>(tail[1] << 8);
    case 1:
      k ^= tail[0];
      k *= c1;
      k = (k << r1) | (k >> (32 - r1));
...
//        getUint32_LE(k4, reinterpret_cast<const byte*>(&blocks[i*4 + 3]), 0);
    ...
    switch (len & 15) {
    case 15: k4 ^= static_cast<uint32>(tail[14]) << 16;
    case 14: k4 ^= static_cast<uint32>(tail[13]) << 8;
    case 13: k4 ^= static_cast<uint32>(tail[12]) << 0;
           k4 *= c4; k4  = ROTL32(k4,18); k4 *= c1; h4 ^= k4;

    case 12: k3 ^= static_cast<uint32>(tail[11]) << 24;
    case 11: k3 ^= static_cast<uint32>(tail[10]) << 16;
    case 10: k3 ^= static_cast<uint32>(tail[ 9]) << 8;
    case  9: k3 ^= static_cast<uint32>(tail[ 8]) << 0;
           k3 *= c3; k3  = ROTL32(k3,17); k3 *= c4; h3 ^= k3;

    case  8: k2 ^= static_cast<uint32>(tail[ 7]) << 24;
    case  7: k2 ^= static_cast<uint32>(tail[ 6]) << 16;
    case  6: k2 ^= static_cast<uint32>(tail[ 5]) << 8;
    case  5: k2 ^= static_cast<uint32>(tail[ 4]) << 0;
           k2 *= c2; k2  = ROTL32(k2,16); k2 *= c3; h2 ^= k2;

    case  4: k1 ^= static_cast<uint32>(tail[ 3]) << 24;
    case  3: k1 ^= static_cast<uint32>(tail[ 2]) << 16;
    case  2: k1 ^= static_cast<uint32>(tail[ 1]) << 8;
    case  1: k1 ^= static_cast<uint32>(tail[ 0]) << 0;
           k1 *= c1; k1  = ROTL32(k1,15); k1 *= c2; h1 ^= k1;
    };
